...
import { professionById } from './tactics-professions'

export const tacticsBoardSize = 35

const makeHero = (id: string, name: string, color: string, stats: TacticsStats, professionId = 'adept', learnedSkillIds: string[] = ['strike']): TacticsHero => ({
  id, name, color, level: 1, xp: 0, training: 0, stats, professionId, attributes: { ...(professionById(professionId)?.initialAttributes ?? { strength: 5, technique: 5, agility: 5, constitution: 5, perception: 5, willpower: 5 }) }, learnedSkillIds, equippedCoreSkillId: learnedSkillIds.find((id) => id.includes('core')) ?? learnedSkillIds.find((id) => id === 'strike') ?? null, equippedTacticalSkillIds: learnedSkillIds.filter((id) => id.includes('tactic') || id === 'bleeding-edge').slice(0, 3), skillDraft: null,
})
...
export const createInitialTacticsSave = (): TacticsSave => ({
  ...
  supplies: 0,
  heroes: [
    makeHero('hero-assassin', '刺客', '#a78bfa', { maxHp: 18, atk: 7, def: 0, move: 4, range: 1, actions: { move: 1, attack: 1 } }, 'assassin', ['strike', 'bleeding-edge']),
