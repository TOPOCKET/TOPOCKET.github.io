...
const parseHero = (data: unknown): TacticsHero => {
  ...
  const learnedSkillIds = record.learnedSkillIds as string[]; const storedCore = record.equippedCoreSkillId === undefined ? null : record.equippedCoreSkillId === null ? null : text(record.equippedCoreSkillId, 'hero.equippedCoreSkillId', 64); const equippedCoreSkillId = storedCore && learnedSkillIds.includes(storedCore) ? storedCore : learnedSkillIds.find((id) => id.endsWith('-core-single') || id === 'strike') ?? null; const equippedTacticalSkillIds = (Array.isArray(record.equippedTacticalSkillIds) ? record.equippedTacticalSkillIds : learnedSkillIds.filter((id) => id.includes('-tactic-') || id === 'bleeding-edge')).filter((id): id is string => typeof id === 'string' && learnedSkillIds.includes(id)).slice(0, 3)
...
const parseUnit = (data: unknown, width: number, height: number): TacticsUnit => {
  ...
  const equippedCoreSkillId = record.equippedCoreSkillId === undefined ? undefined : record.equippedCoreSkillId === null ? null : text(record.equippedCoreSkillId, 'unit.equippedCoreSkillId', 64)
  if (equippedCoreSkillId && !learnedSkillIds?.includes(equippedCoreSkillId)) throw new Error('unit.equippedCoreSkillId must be learned')
  if (record.equippedTacticalSkillIds !== undefined && (!Array.isArray(record.equippedTacticalSkillIds) || record.equippedTacticalSkillIds.length > 3 || !record.equippedTacticalSkillIds.every((id) => typeof id === 'string' && learnedSkillIds?.includes(id)))) throw new Error('unit.equippedTacticalSkillIds is invalid')
  const equippedTacticalSkillIds = record.equippedTacticalSkillIds as string[] | undefined
