...
import type { TacticsAttributeName, TacticsHero, TacticsPoint, TacticsSkill, TacticsStats, TacticsUnit } from '../model/tactics-model'
...
export const useTacticsGame = () => {
  ...
  const train = (hero: TacticsHero, stat: TacticsAttributeName) => { save.value = trainHeroStat(save.value, hero.id, stat); message.value = `${hero.name} 的${({ strength: '力量', technique: '技巧', agility: '敏捷', constitution: '体质', perception: '感知', willpower: '意志' } as const)[stat]}提升了。` }
